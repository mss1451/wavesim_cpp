...
protected:
	...
	unsigned int * osc_locations_size; // For each oscillator, how many index values are there?
	...
	unsigned int amplitudeMultiplier = 20; // Multiplied by height at painting stage to reveal weaker vibrations.
	...
	unsigned int size = 300; // Size of the wave pool. It indicates both the width and height since the pool will always be a square.
	...
	unsigned int absorb_offset = 25; // Offset from each window boundary where the loss starts to increase.
	double max_loss = 0.3; // The highest loss value. They are located at the boundaries.
	bool absorberEnabled = true; // If true, the particles near the boundaries will have high loss.
...
public:
	...
	void setOscillatorLocation(unsigned int oscillatorId,
	...
	// Not any function must be called except 'getData' and 'unlock' otherwise the process will hang.
	bool lock();
