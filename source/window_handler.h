...
protected:
	...
	SpinButton * spinbutton_size, *spinbutton_loss, *spinbutton_abs_peak,
			...
			*spinbutton_amp, *spinbutton_locx1, *spinbutton_locy1,
			*spinbutton_locx2, *spinbutton_locy2, *spinbutton_move_period,
			*spinbutton_mass_pen, *spinbutton_primary_mass,
			*spinbutton_secondary_mass, *spinbutton_mass_min,
			*spinbutton_mass_max, *spinbutton_static_pen,
			*spinbutton_max_iterate, *spinbutton_num_co,
	...
	int showMsg(ustring msg, MessageType msgType, ButtonsType buttonsType,
			Window * transient);
	template<typename array, typename value> void iterateShape(array, value,
			Point p1, Point p2, int x, int y, double radius, int size);
	void refresh_scene();
