...
enum ProjectHandlerResult{
	OK,
	NOT_FOUND,
	ERROR
};

// Takes a pointer to a WaveEngine object and saves it to or modifies it from a project file.
...
public:
	...
	ProjectHandlerResult openProject(string projectName, string & description, WaveEngine * waveEngine);
	...
	ProjectHandlerResult saveProject(string projectName, string description, WaveEngine * waveEngine);

	// Deletes a wave simulator project.\n
	// Path must not be included as all projects are found in a certain folder.\n
	ProjectHandlerResult deleteProject(string projectName);

	// Returns a string array containing the list of project names found under the project folder.\n
	// Returns null if not found any projects or there is an error.
	...
	void cleanOnError(bool wasWorking, FILE * pFile, WaveEngine * waveEngine, string path_uncompressed);
