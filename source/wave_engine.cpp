...
WaveEngine::WaveEngine() {
  coThreads = (std::thread *)calloc(sizeof(std::thread), MAX_NUMBER_OF_THREADS);
  ctStruct =
      (coThreadStruct *)calloc(sizeof(coThreadStruct), MAX_NUMBER_OF_THREADS);
  mCT = (std::mutex *)calloc(sizeof(std::mutex), MAX_NUMBER_OF_THREADS);
  mCTC = (std::condition_variable *)calloc(sizeof(std::condition_variable),
  ...
  osc_amplitude = (double *)calloc(sizeof(double), MAX_NUMBER_OF_OSCILLATORS);
  for (unsigned int i = 0; i < MAX_NUMBER_OF_OSCILLATORS; i++)
    osc_amplitude[i] = 1.0;
  osc_move_period = (double *)calloc(sizeof(double), MAX_NUMBER_OF_OSCILLATORS);
  for (unsigned int i = 0; i < MAX_NUMBER_OF_OSCILLATORS; i++)
    osc_move_period[i] = 800.0;
  osc_locations = (unsigned int **)calloc(sizeof(unsigned int *),
                                          MAX_NUMBER_OF_OSCILLATORS);
  osc_locations_size =
      (unsigned int *)calloc(sizeof(unsigned int), MAX_NUMBER_OF_OSCILLATORS);
  osc_location1_p = (Point *)calloc(sizeof(Point), MAX_NUMBER_OF_OSCILLATORS);
  osc_location2_p = (Point *)calloc(sizeof(Point), MAX_NUMBER_OF_OSCILLATORS);
...
void WaveEngine::sendOrderToCT(coThreadMission order) {
  ...
  // mutexedCout("order thread: making busy");
  ctMission = order;
  for (unsigned int i = 0; i < numOfThreads; i++) {
    std::unique_lock<std::mutex> lk(mCT[i]);
    ctStruct[i].busy = true;
  }
  for (unsigned int i = 0; i < numOfThreads; i++)
    mCTC[i].notify_one();
  // mutexedCout("order thread: waiting to be done");
  for (unsigned int i = 0; i < numOfThreads; i++) {
    std::unique_lock<std::mutex> lk(mCT[i]);
    while (ctStruct[i].busy && !disposing && work_now)
      mCTC[i].wait_for(lk, std::chrono::seconds(1));
  }
  // mutexedCout("order thread: exiting");
}

void WaveEngine::MainThreadFunc(void *data) {
  WaveEngine *waveEngine = (WaveEngine *)data;

  // It is challenging to be accurate with the limiter at high speed
  // calculations. If we use thread.sleep() function, number of iterations per
  // second (IPS) will be quite lower than the one specified for the limiter at
  // high speeds. That is because, the sleep times are not perfect and the
  // program doesn't always get enough CPU power. So we take another approach.
  // We calculate the number of the calculations that should have been done
  // since the epoch by checking the current time-stamp and decide if the main
  // thread should yield or do another iteration. That way, the calculation
  // periods will not be always homogeneous and there will be bursting problem
  // but at least the requested number of calculations will be provided.
  ...
  time_previous =
      time_current -
      std::chrono::seconds(1); // Make sure that a new session starts first.
  double numOfCalcs =
      0; // For statistics, how many calculations have been done so far?
  double numOfPaints =
      0; // For statistics, how many paintings have been done so far?
  unsigned int calcNeeded =
      0; // How many calculations should have been done since epoch?
  unsigned int paintNeeded =
      0; // How many paintings should have been done since epoch?
  while (!waveEngine->disposing) {

    while (waveEngine->work_now) {

      // Reset the counters every second. If calcDone can't keep up with the
      // calcNeeded for some reason (lag, etc.) and the difference between them
      // is too big, then the program will ignore the limiter for a long time,
      // trying to reach the calcNeeded (too long bursts). With this session
      // thing, the program will ignore the limiter at most for a second.
      ...
      if (waveEngine->work_now) {
        time_current = std::chrono::steady_clock::now();
        if (std::chrono::duration_cast<std::chrono::duration<double>>(
                time_current - time_log_previous)
                .count() >= waveEngine->performanceLogInterval / 1000.0) {
          time_log_previous = std::chrono::steady_clock::now();
          double perf_interval = (double)waveEngine->performanceLogInterval;
          waveEngine->mCout.lock();
          std::cout << "Iterations & Paints per second:	"
                    << 1000.0 / (perf_interval / numOfCalcs) << "	"
                    << 1000.0 / (perf_interval / numOfPaints) << std::endl;
          numOfCalcs = 0;
          numOfPaints = 0;
...
Point WaveEngine::getOscillatorRealLocation(unsigned int oscillatorId) {
  if (oscillatorId < MAX_NUMBER_OF_OSCILLATORS) {
    ...
    case LineSource:
      return Point(
          (osc_location1_p[oscillatorId].x + osc_location2_p[oscillatorId].x) /
              2,
          (osc_location1_p[oscillatorId].y + osc_location2_p[oscillatorId].y) /
              2);
    case MovingPointSource:
      ratio_1 = fmod(calcCounter, osc_move_period[oscillatorId]) /
                osc_move_period[oscillatorId];
      return Point((1.0 - ratio_1) * osc_location1_p[oscillatorId].x +
...
double WaveEngine::getLossRatio() { return loss; }
...
double WaveEngine::getFramesPerSecond() { return FPS; }
...
double WaveEngine::getIterationsPerSecond() { return IPS; }
...
unsigned int WaveEngine::getNumberOfThreads() { return numOfThreads; }

void WaveEngine::setNumberOfThreads(unsigned int numberOfThreads) {
  mutex.lock();
  setCoThreads(clamp(numberOfThreads, 1, MAX_NUMBER_OF_THREADS));
  mutex.unlock();
}

unsigned int WaveEngine::getThreadDelay() { return TDelay; }
...
bool WaveEngine::getRenderEnabled() { return renderEnabled; }
...
bool WaveEngine::getCalculationEnabled() { return calculationEnabled; }
...
bool WaveEngine::getLogPerformance() { return logPerformance; }
...
bool WaveEngine::getPowerSaveMode() { return powerSaveMode; }
...
void WaveEngine::setPerformanceLogInterval(
  ...
  this->performanceLogInterval =
      clamp(performanceLogInterval, 0, performanceLogInterval);
  mutex.unlock();
}

double WaveEngine::getMassMapRangeHigh() { return massMapRangeHigh; }
...
double WaveEngine::getMassMapRangeLow() { return massMapRangeLow; }
...
bool WaveEngine::getShowMassMap() { return massMap; }
...
unsigned int WaveEngine::getSize() { return size; }
...
double WaveEngine::getAbsorberLossRatio() { return max_loss; }
...
unsigned int WaveEngine::getAbsorberThickness() { return absorb_offset; }
...
bool WaveEngine::getShiftParticlesEnabled() { return shifting; }
...
bool WaveEngine::getAbsorberEnabled() { return absorberEnabled; }
...
void *WaveEngine::getExtraData() { return extra_data; }

bool WaveEngine::getExtremeContrastEnabled() { return extremeContrastEnabled; }
...
Color WaveEngine::getCrestColor() { return crestColor; }
...
Color WaveEngine::getTroughColor() { return troughColor; }
...
Color WaveEngine::getStaticColor() { return staticColor; }
...
bool WaveEngine::isWorking() { return work_now; }
bool WaveEngine::calculateForces(const unsigned int firstIndex,
  ...
  for (unsigned int index = firstIndex; index < fplusc; index++) {
    // If this is a static particle, it will not move at all. Continue with the
    // next particle.
    if (pd_static[index]) {
      pd[index] = 0;
      continue;
    ...
    // Think that each particle is connected to its 4 neighbor particles
    // (north,south,east,west). The connection is provided by a spring whose
    // constant is k = 1 The particles can only move up and down meaning that
    // only their altitude/height can change. Find forces exerted by springs. F
    // = -kx in this case. x is the distance to the rest point of that spring.
    // Springs are connected in parallel. So forces are simply added.
    // After finding the total force, from F = ma -> F/m = a, find the
    // acceleration, and add it to the velocity.
    ...
    if (num_of_parts != 0) {
      ...
      double velocity_gain =
          (heights - pd_previous[index] * num_of_parts) / pdm[index];

      // Limit velocity to prevent chaos. When the simulation resolution is low
      // enough, a particle can gain a velocity which would carry it to such a
      // point in single iteration that it would be impossible if the simulation
      // resolution would be higher. If not prevented, the particle will vastly
      // increase its distance from the equilibrium point. This is a positive
      // feedback case resulting in violation of conservation of energy and,
      // ultimately, the chaos.
      if (velocity_gain > 0)
        pd[index] += pdv[index] += clamp(velocity_gain, velocity_gain,
                                         (average_height - pd_previous[index]));
      else
        pd[index] += pdv[index] +=
            clamp(velocity_gain, (average_height - pd_previous[index]),
                  velocity_gain);
    }
    ...
    if (pdl[index] > 0.0) {
      // Reduce the kinetic energy. The kinetic energy is 0.5 times mass times
      // velocity squared. With this equation, we can derive the velocity for
      // the reduced kinetic energy.
      double kinetic_energy = (0.5 * pdm[index] * pow(pdv[index], 2));
      // Multiply the energy with one minus loss ratio and find the velocity for
      // that kinetic energy. Leaving the velocity on the left side results in
      // the following.
      pdv[index] = sqrt(2 * kinetic_energy * (1.0 - pdl[index]) / pdm[index]) *
                   sgn(pdv[index]);
      ...
      /*double height_difference = pd_previous[index]
       ...
       // Reduce the potential energy. The potential energy for this model is
       0.5 times height difference squared.
       // With this equation, we can derive the height for the reduced potential
       energy. double potential_energy = (0.5 * pow(height_difference, 2.0));
       // Multiply the energy with one minus loss ratio and find the height for
       that potential energy.
       // Leaving the height difference on the left side results in the
       following. pd[index] += sqrt(2 * potential_energy * (1.0 - pdl[index]))
  ...
  for (unsigned int i = 0; i < MAX_NUMBER_OF_OSCILLATORS; i++) {
    if (osc_active[i]) {
      double osc_height =
          osc_amplitude[i] *
          sin(osc_phase[i] * M_PI / 180.0 +
              2.0 * M_PI * fmod(calcCounter, osc_period[i]) / osc_period[i]);
      ...
      case MovingPointSource:
        double ratio_1 =
            fmod(calcCounter, osc_move_period[i]) / osc_move_period[i];
        Point cur_point = Point((1.0 - ratio_1) * osc_location1_p[i].x +
                                    ratio_1 * osc_location2_p[i].x,
                                (1.0 - ratio_1) * osc_location1_p[i].y +
                                    ratio_1 * osc_location2_p[i].y);
        unsigned int cur_index =
            (unsigned int)cur_point.x + size * (unsigned int)cur_point.y;
...
void WaveEngine::setLossRatio() {
  if (absorberEnabled) {
    // We will fill "pdl" array with "loss" then we will deal with elements near
    // to window boundaries.

    // Since we want the loss to increase towards the edges, "max_loss" can't be
    // smaller than "loss".
    if (max_loss < loss) {
    ...
    // loss gain fields should not mix with each other. So the maximum offset is
    // the middle-screen.
    if (absorb_offset >= size / 2) {
      absorb_offset = size / 2 - 1;
    }

    // This value is loss increasion rate per row/column. The increasion is
    // linear.
...
bool WaveEngine::paintBitmap(const unsigned int firstIndex,
  ...
  for (unsigned int index = firstIndex; index < firstIndex + count; index++) {
    if (!massMap) {
      ...
      if (pd_static[index]) {
        ...
        if (extremeContrastEnabled) {
          if (pdx > 0) {
            ...
            rgbdata[index * 3] =
                (uint8_t)((crestColor.r + troughColor.r) / 2.0);
            rgbdata[index * 3 + 1] =
                (uint8_t)((crestColor.g + troughColor.g) / 2.0);
            rgbdata[index * 3 + 2] =
                (uint8_t)((crestColor.b + troughColor.b) / 2.0);
          }
        } else {
          double bright =
              (clamp(pdx * amplitudeMultiplier, -1.0, 1.0) + 1.0) * 127.0;

          double brightr1 = bright / 255.0;
          double brightr2 = 1.0 - brightr1;

          rgbdata[index * 3] =
              crestColor.r * brightr1 + troughColor.r * brightr2;
          rgbdata[index * 3 + 1] =
              crestColor.g * brightr1 + troughColor.g * brightr2;
          rgbdata[index * 3 + 2] =
              crestColor.b * brightr1 + troughColor.b * brightr2;
        }
      }
    } else {
      ...
      double colors =
          128.0 * 5.0 - 4.0; // numFrames * numTrans - (numTrans - 1)
      if (massrange <= 0) {
        rgbdata[index * 3] = 0;
        rgbdata[index * 3 + 1] = 0;
        rgbdata[index * 3 + 2] = 0;
      ...
      uint16_t color =
          round((clamp(pdm[index], massMapRangeLow, massMapRangeHigh) -
                 massMapRangeLow) *
                colors / massrange);
      if (color < 128) {
        rgbdata[index * 3] = 0;
...
void WaveEngine::setPool(unsigned int oldsize) {
  ...
  if (!pd_static_null) {
    ...
    memcpy(pd_static_old, pd_static, sizeof(pd_static_old[0]) * oldsizesize);
  }
  ...
  free(pd_location_info);
  pd_location_info =
      (ParticleLocationInfo *)calloc(sizeof(ParticleLocationInfo), sizesize);
  ...
  if (!pd_static_null || !pdm_null) {
    ...
    for (unsigned int y = 0; y < size; y++) {
      for (unsigned int x = 0; x < size; x++) {
        if (!pd_static_null)
          pd_static[x + size * y] =
              pd_static_old[(unsigned int)floor((double)x * stepsize +
                                                stepsize_db2) +
                            oldsize * (unsigned int)floor((double)y * stepsize +
                                                          stepsize_db2)];
        if (!pdm_null)
          pdm[x + size * y] =
              pdm_old[(unsigned int)floor((double)x * stepsize + stepsize_db2) +
                      oldsize * (unsigned int)floor((double)y * stepsize +
                                                    stepsize_db2)];
      }
    }
  ...
  for (unsigned int i = 0; i < MAX_NUMBER_OF_OSCILLATORS; i++) {
    osc_location1_p[i] = Point(osc_location1_p[i].x * (double)size / oldsize,
                               osc_location1_p[i].y * (double)size / oldsize);

    osc_location2_p[i] = Point(osc_location2_p[i].x * (double)size / oldsize,
...
void WaveEngine::updateOscLocIndices(unsigned int oscillatorId) {
  ...
  case PointSource:
    if (p1.x >= 0 && p1.x < size && p1.y >= 0 && p1.y < size) {
      ...
      osc_locations[oscillatorId][0] =
          (unsigned int)p1.x + size * (unsigned int)p1.y;
    }
    break;
  case LineSource:
    ...
    for (double i = 0; i < length; i += 0.5) {
      ...
      if (currentPoint.x < size && currentPoint.x >= 0 &&
        ...
        indices.push_back((unsigned int)floor(currentPoint.x) +
    ...
    if ((osc_locations_size[oscillatorId] = indices.size()) > 0) {
      osc_locations[oscillatorId] = (unsigned int *)calloc(
          sizeof(unsigned int), osc_locations_size[oscillatorId]);
      std::copy(indices.begin(), indices.end(), osc_locations[oscillatorId]);
    }
    break;
  case MovingPointSource:
    free(osc_locations[oscillatorId]);
...
void WaveEngine::CoThreadFunc(void *data) {
  ...
  while (waveEngine->ctMission != Destroy && !waveEngine->disposing) {
    {
      std::unique_lock<std::mutex> lk(waveEngine->mCT[i]);

      while (!waveEngine->ctStruct[i].busy && !waveEngine->disposing &&
             waveEngine->work_now) {
        waveEngine->mCTC[i].wait_for(lk, std::chrono::seconds(1));
      }
      ...
      if (waveEngine->ctMission == CalculateForces) {
        ...
        waveEngine->paintBitmap(waveEngine->ctStruct[i].firstIndex,
                                waveEngine->ctStruct[i].count,
                                waveEngine->bitmap_data);
      }

      waveEngine->ctStruct[i].busy = false;
    }
    waveEngine->mCTC[i].notify_one();
...
void WaveEngine::setCoThreads(unsigned int newNumOfThreads) {
  ...
  for (unsigned int i = 0; i < numOfThreads; i++) {
    ...
    pthread_setaffinity_np(coThreads[i].native_handle(), sizeof(cpu_set_t),
                           &cpuset);
    /*if (pthread_getaffinity_np(coThreads[i].native_handle(),
   sizeof(cpu_set_t), &cpuset) == 0){ for (int j = 0; j < CPU_SETSIZE; j++) if
   (CPU_ISSET(j, &cpuset)){ mCout.lock(); std::cout << "Co-thread(" << i << ")
   created on core(" << j <<")\n"; mCout.unlock();
               }
    }*/
...
void WaveEngine::mutexedCout(const char *msg) {
  mCout.lock();
