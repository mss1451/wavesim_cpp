...
WaveEngine::WaveEngine()
{
	...
	ctStruct = (coThreadStruct *)calloc(sizeof(coThreadStruct),
										MAX_NUMBER_OF_THREADS);
	mCT = (std::mutex *)calloc(sizeof(std::mutex),
							   MAX_NUMBER_OF_THREADS);
...
void WaveEngine::MainThreadFunc(void *data)
{
	...
	while (!waveEngine->disposing)
	{
		...
		while (waveEngine->work_now)
		{
			...
			{
				time_current = std::chrono::steady_clock::now();
				if ((waveEngine->IPS == 0 || (calcNeeded =
												  (unsigned int)(std::chrono::duration_cast<
																	 std::chrono::duration<double>>(
																	 time_current.time_since_epoch())
																	 .count() *
																 waveEngine->IPS)) > waveEngine->calcDone))
				{
			...
			{
				time_current = std::chrono::steady_clock::now();
				if (waveEngine->FPS == 0 || (paintNeeded =
												 (unsigned int)(std::chrono::duration_cast<
																	std::chrono::duration<double>>(
																	time_current.time_since_epoch())
																	.count() *
																waveEngine->FPS)) > waveEngine->paintDone)
				{
					waveEngine->mutex.lock();
...
Point WaveEngine::getOscillatorRealLocation(unsigned int oscillatorId)
{
	if (oscillatorId < MAX_NUMBER_OF_OSCILLATORS)
	{
		...
		case LineSource:
			return Point(
				(osc_location1_p[oscillatorId].x + osc_location2_p[oscillatorId].x) / 2,
				(osc_location1_p[oscillatorId].y + osc_location2_p[oscillatorId].y) / 2);
		case MovingPointSource:
			ratio_1 = fmod(calcCounter, osc_move_period[oscillatorId]) / osc_move_period[oscillatorId];
			return Point(
				(1.0 - ratio_1) * osc_location1_p[oscillatorId].x + ratio_1 * osc_location2_p[oscillatorId].x,
				(1.0 - ratio_1) * osc_location1_p[oscillatorId].y + ratio_1 * osc_location2_p[oscillatorId].y);
		default:
			return Point(-1, -1);
		}
	}
								 ...
								 const unsigned int count)
{
	...
	for (unsigned int index = firstIndex; index < fplusc; index++)
	{
		...
		if (num_of_parts != 0)
		{
			...
			double velocity_gain = (heights - pd_previous[index] * num_of_parts) / pdm[index];
			...
			if (velocity_gain > 0)
				...
				pd[index] += pdv[index] += clamp(velocity_gain,
												 (average_height - pd_previous[index]), velocity_gain);
		}
	...
	for (unsigned int i = 0; i < MAX_NUMBER_OF_OSCILLATORS; i++)
	{
		if (osc_active[i])
		{
			double osc_height = osc_amplitude[i] * sin(
													   osc_phase[i] * M_PI / 180.0 + 2.0 * M_PI * fmod(calcCounter, osc_period[i]) / osc_period[i]);
			...
			case LineSource:
				...
				break;
			case MovingPointSource:
				double ratio_1 = fmod(calcCounter, osc_move_period[i]) / osc_move_period[i];
				Point cur_point = Point(
					(1.0 - ratio_1) * osc_location1_p[i].x + ratio_1 * osc_location2_p[i].x,
					(1.0 - ratio_1) * osc_location1_p[i].y + ratio_1 * osc_location2_p[i].y);
				unsigned int cur_index = (unsigned int)cur_point.x + size * (unsigned int)cur_point.y;
	...
	for (unsigned int index = firstIndex; index < firstIndex + count; index++)
	{
			...
			else
			{
				...
				if (extremeContrastEnabled)
				{
					...
					else if (pdx == 0)
					{
						...
						rgbdata[index * 3] = (uint8_t)((crestColor.r + troughColor.r) / 2.0);
						rgbdata[index * 3 + 1] = (uint8_t)((crestColor.g + troughColor.g) / 2.0);
						rgbdata[index * 3 + 2] = (uint8_t)((crestColor.b + troughColor.b) / 2.0);
					}
				}
				else
				{
					double bright = (clamp(pdx * amplitudeMultiplier, -1.0, 1.0) + 1.0) * 127.0;

					double brightr1 = bright / 255.0;
					double brightr2 = 1.0 - brightr1;

					rgbdata[index * 3] = crestColor.r * brightr1 + troughColor.r * brightr2;
					rgbdata[index * 3 + 1] = crestColor.g * brightr1 + troughColor.g * brightr2;
					rgbdata[index * 3 + 2] = crestColor.b * brightr1 + troughColor.b * brightr2;
				}
			}
		}
		else
		{
			...
			uint16_t color = round(
				(clamp(pdm[index], massMapRangeLow, massMapRangeHigh) - massMapRangeLow) * colors / massrange);
			if (color < 128)
			{
				rgbdata[index * 3] = 0;
				rgbdata[index * 3 + 1] = 0;
...
void WaveEngine::updateOscLocIndices(unsigned int oscillatorId)
{
	...
	case PointSource:
		if (p1.x >= 0 && p1.x < size && p1.y >= 0 && p1.y < size)
		{
			...
			osc_locations[oscillatorId][0] = (unsigned int)p1.x + size * (unsigned int)p1.y;
		}
		break;
	case LineSource:
		...
		for (double i = 0; i < length; i += 0.5)
		{
			...
			if (currentPoint.x < size && currentPoint.x >= 0 && currentPoint.y < size && currentPoint.y >= 0)
				indices.push_back(
					(unsigned int)floor(currentPoint.x) + size * (unsigned int)floor(currentPoint.y));
		}
		free(osc_locations[oscillatorId]);
	...
	{
		{
			...
			while (!waveEngine->ctStruct[i].busy && !waveEngine->disposing && waveEngine->work_now)
			{
...
void WaveEngine::setCoThreads(unsigned int newNumOfThreads)
{
	...
	for (unsigned int i = 0; i < numOfThreads; i++)
	{
		...
		pthread_setaffinity_np(coThreads[i].native_handle(), sizeof(cpu_set_t), &cpuset);
		/*if (pthread_getaffinity_np(coThreads[i].native_handle(), sizeof(cpu_set_t), &cpuset) == 0){
			for (int j = 0; j < CPU_SETSIZE; j++)
               if (CPU_ISSET(j, &cpuset)){
				   mCout.lock();
				   std::cout << "Co-thread(" << i << ") created on core(" << j <<")\n";
				   mCout.unlock();
			   }
		}*/
