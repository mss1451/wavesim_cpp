...
string ProjectHandler::getProjectsPath() { return project_dir; }
...
ProjectHandlerResult ProjectHandler::saveProject(string projectName,
  ...
  if ((pFile = fopen(path_uncompressed.c_str(), "wb")) != nullptr) {
    ...
    double_t *osc_period = (double_t *)calloc(osc_count, sizeof(double_t));
    double_t *osc_phase = (double_t *)calloc(osc_count, sizeof(double_t));
    double_t *osc_amplitude = (double_t *)calloc(osc_count, sizeof(double_t));
    double_t *osc_move_period = (double_t *)calloc(osc_count, sizeof(double_t));
    ...
    result |=
        smart_fwrite(&absorber_thickness, sizeof(absorber_thickness), 1, pFile);
    result |= smart_fwrite(&shift_to_origin, sizeof(shift_to_origin), 1, pFile);
    result |= smart_fwrite(&enable_absorber, sizeof(enable_absorber), 1, pFile);
    ...
    for (uint64_t i = 0; i < osc_count; i++) {
      ...
      result |= smart_fwrite(&osc_active[i], sizeof(osc_active[0]), 1, pFile);
      result |= smart_fwrite(&osc_source[i], sizeof(osc_source[0]), 1, pFile);
      result |= smart_fwrite(&osc_period[i], sizeof(osc_period[0]), 1, pFile);
      result |= smart_fwrite(&osc_phase[i], sizeof(osc_phase[0]), 1, pFile);
      result |=
          smart_fwrite(&osc_amplitude[i], sizeof(osc_amplitude[0]), 1, pFile);
      result |= smart_fwrite(&osc_move_period[i], sizeof(osc_move_period[0]), 1,
                             pFile);
      result |= smart_fwrite(&osc_location1_p[i], sizeof(osc_location1_p[0]), 1,
                             pFile);
      result |= smart_fwrite(&osc_location2_p[i], sizeof(osc_location2_p[0]), 1,
                             pFile);
    }
...
ProjectHandlerResult ProjectHandler::openProject(string projectName,
  ...
  if ((pFile = fopen(path_uncompressed.c_str(), "rb")) != nullptr) {
    ...
    result |=
        smart_fread(&absorber_thickness, sizeof(absorber_thickness), 1, pFile);
    result |= smart_fread(&shift_to_origin, sizeof(shift_to_origin), 1, pFile);
    result |= smart_fread(&enable_absorber, sizeof(enable_absorber), 1, pFile);
    ...
    for (uint64_t i = 0; i < osc_count; i++) {
      result |= smart_fread(&osc_active[i], sizeof(osc_active[0]), 1, pFile);
      result |= smart_fread(&osc_source[i], sizeof(osc_source[0]), 1, pFile);
      result |= smart_fread(&osc_period[i], sizeof(osc_period[0]), 1, pFile);
      result |= smart_fread(&osc_phase[i], sizeof(osc_phase[0]), 1, pFile);
      result |=
          smart_fread(&osc_amplitude[i], sizeof(osc_amplitude[0]), 1, pFile);
      result |= smart_fread(&osc_move_period[i], sizeof(osc_move_period[0]), 1,
                            pFile);
      result |= smart_fread(&osc_location1_p[i], sizeof(osc_location1_p[0]), 1,
                            pFile);
      result |= smart_fread(&osc_location2_p[i], sizeof(osc_location2_p[0]), 1,
                            pFile);
    ...
    if (length > 0) {
      char *description_cstring =
          (char *)calloc(length + 1, 1); // Include null terminator
...
ProjectHandlerResult ProjectHandler::getProjectsList(vector<string> &projects) {
  ...
  while ((pDirEnt = readdir(pDir)) != NULL) {
    // std::cout << pDirEnt->d_name << ", " << (int)pDirEnt->d_type <<
    // std::endl;
...
ProjectHandlerResult ProjectHandler::smart_fread(void *data, size_t elementSize,
                                                 size_t numberOfElements,
                                                 FILE *pFile) {
